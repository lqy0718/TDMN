...
def extract_one(task):
    ...
    try:
        ...
        # CRTS MJD values are positive. A few catalogue files contain the
        # sentinel -2409998.5, which makes UPSILoN allocate an enormous
        # Lomb-Scargle frequency grid and can raise MemoryError.
        if SURVEY == "crts":
...
def source_has_invalid_crts_time(path):
    """Detect non-positive CRTS MJD sentinels without loading a full table."""
    if SURVEY != "crts":
        return False
    try:
...
def main():
    ...
    parser.add_argument(
        "--manifest",
        default=str(REPO_ROOT / "data/processed/crts_tdmn.manifest.csv"),
        help="Real-object manifest emitted by the matching TDMN data preparation",
    )
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "data/processed/crts_upsilon_16_features.csv"),
    )
    ...
    if args.resume and output_path.is_file():
        ...
        existing["extraction_error"] = existing["extraction_error"].fillna("").astype(str)
        existing = existing.drop_duplicates(subset=["object_id"], keep="last")
        retry_mask = existing["extraction_error"] != ""
        invalid_time_mask = existing["source_path"].map(source_has_invalid_crts_time)
        retry_mask |= invalid_time_mask
        if retry_mask.any():
            print(
                f"resume: keeping {int((~retry_mask).sum())} successful objects; "
                f"retrying {int(retry_mask.sum())} failed or invalid-time objects",
                flush=True,
            )
        existing = existing.loc[~retry_mask].copy()
    ...
    if tasks:
        effective_workers = 1 if len(existing) and len(tasks) <= 16 else min(args.workers, len(tasks))
        print(f"extracting {len(tasks)} objects with {effective_workers} worker(s)", flush=True)
    ...
    frame["extraction_error"] = frame["extraction_error"].fillna("").astype(str)
    if frame["object_id"].duplicated().any():
        raise ValueError("Feature output contains duplicate object_id values")
    expected_ids = set(full_manifest["object_id"].astype(str))
    actual_ids = set(frame["object_id"].astype(str))
    if actual_ids != expected_ids:
        raise ValueError(
            f"Feature output object mismatch: missing={len(expected_ids - actual_ids)}, "
            f"extra={len(actual_ids - expected_ids)}"
        )
    frame = frame.set_index("object_id").loc[full_manifest["object_id"].astype(str)].reset_index()
    ...
    report = {
        "survey": SURVEY,
        "objects_in_manifest": int(len(full_manifest)),
