...
V5_SOURCE_SHA256 = "ebe98c6390f8080a4b915ecf80019d2d133a4a6879d88302a418988857715da9"
...
def build_pure_di_modal_encoder(seq_shape, img_shape):
    ...
    for _ in range(2):
        attn = layers.MultiHeadAttention(num_heads=8, key_dim=16, dropout=0.2)(x_seq, x_seq, attention_mask=attn_mask)
    ...
    for filters, do_rate in [(64, 0.05), (128, 0.05), (256, 0.1)]:
        ...
        if filters != 256: x_img = layers.MaxPooling2D(2)(x_img)

    img_repr = layers.Concatenate()([layers.GlobalAveragePooling2D()(x_img), layers.GlobalMaxPooling2D()(x_img)])
...
# --- Exp1 data processing: global photometric noise ---
def preprocess(seq, img, feat, mask, label):
    ...
    seq_processed = tf.stack([raw_mag, smooth_mag, err, phase_sin, phase_cos, phase, is_valid], axis=-1)
    return {"seq_in": seq_processed, "img_in": img, "feat_in": feat}, label

def augment_global_noise(inputs, label):
    seq = inputs["seq_in"]
    raw_mag, smooth_mag, err, phase_sin, phase_cos, phase, valid_mask = tf.unstack(seq, axis=-1)
    # Exp1 applies the same photometric noise rule to every class.
    mag_noise = tf.random.normal(tf.shape(raw_mag), stddev=0.02) * valid_mask
...
def create_dataset(indices, X_seq, X_img, X_feat, X_mask, y_oh, feat_dim, is_train=False):
    ...
    if is_train:
        ...
        ds = ds.map(augment_global_noise, num_parallel_calls=tf.data.AUTOTUNE)
        ds = ds.shuffle(8192)
    return ds.batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)

# --- 模型构建 ---
def build_exp1_model(seq_shape, img_shape, feat_dim):
    ...
    feat_repr = layers.Dense(64, kernel_regularizer=regularizers.l2(1.5e-4))(feat_in)
    feat_repr = layers.BatchNormalization()(feat_repr)
    feat_repr = layers.Activation("swish")(feat_repr)

    stacked = layers.SpatialDropout1D(0.15)(tf.stack([seq_repr, img_repr], axis=1))
    fusion = layers.Dense(128, kernel_regularizer=regularizers.l2(1.5e-4))(layers.Reshape((256,))(stacked))
    fusion = layers.Dropout(0.5)(layers.Activation("swish")(layers.BatchNormalization()(fusion)))

    final_features = layers.Concatenate()([fusion, feat_repr])
    output = layers.Dense(NUM_CLASSES, activation="softmax", name="standard_head")(final_features)
...
def run_exp1():
    ...
    model = build_exp1_model((tr["seq"].shape[1], 7), (128, 128, 3), feat_dim)
    ...
    checkpoint = str(output / "baseline_best.ckpt")
    ...
    _finish_run(output, data_stat, config, te["y"], probabilities, te["object_id"],
                selection, "V5 Exp 1, seed=" + str(SEED), "Blues")
