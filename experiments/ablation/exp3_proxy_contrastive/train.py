...
V5_SOURCE_SHA256 = "ea19e7cffbc33a421325420081baa1048f9fa0715c3fa4e2a241ce255e8fd774"
...
# ==========================================
# 3. Keras Subclass Model (修复了变量名和输入层)
# ==========================================
class ProxyContrastiveE2EModel(keras.Model):
    def __init__(self, encoder, feat_dim, num_classes, **kwargs):
        ...
        # 🚨 [关键修复]: 所有自定义变量必须赋 name，否则 ModelCheckpoint 会崩溃
        self.temperature = tf.Variable(
            0.1, trainable=False, dtype=tf.float32, name="temperature"
        )

        # 🚨 [关键修复]: 必须指定 input_shape 并赋 name
        ...
        self.classifier = layers.Dense(
            num_classes, activation="softmax", name="standard_head"
        )

        # 🚨 [关键修复]: 强制 build 图结构
        self.fusion_dense.build((None, 256))
        self.feat_encoder.build((None, self.feat_dim))
        self.classifier.build((None, 192))  # 128 (fusion) + 64 (feat) = 192
        ...
        if self.current_stage == 1:
            ...
            loss = tf.reduce_mean(
                tf.keras.losses.categorical_crossentropy(
                    y, self.classifier(layers.Concatenate()([fusion, feat_repr]))
                )
            )
...
def run_exp3():
    ...
    model = ProxyContrastiveE2EModel(encoder, feat_dim, NUM_CLASSES)
    ...
    model.feat_encoder.trainable = True
    ...
    _finish_run(output, data_stat, config, te["y"], probabilities, te["object_id"],
                selection, "V5 Exp 3, seed=" + str(SEED), "Purples")
